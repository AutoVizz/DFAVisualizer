...
export default function AlphabetEditor() {
  ...
  const addSymbol = () => {
    ...
    if (sym === "ε") {
      setError("No need to add ε — it's handled automatically.");
      return;
    }
    if (activeProject.alphabet.includes(sym)) {
      setError("That symbol is already in your alphabet.");
      return;
    }
    ...
    <div className="sidebar-section">
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 10 }}>
        {activeProject.alphabet.map((sym) => (
          <span key={sym} className="alpha-chip">
            {sym}
            <button onClick={() => removeSymbol(sym)} title="Remove">
              ✕
            </button>
          </span>
        ))}
      ...
      <div style={{ display: "flex", gap: 6 }}>
        <input
          ...
          style={{ flex: 1 }}
        />
        <button className="btn btn-ghost btn-sm" onClick={addSymbol}>
          Add
        </button>
      </div>
