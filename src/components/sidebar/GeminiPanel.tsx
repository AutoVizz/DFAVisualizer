...
function probeTable(results: ProbeResult[]): string {
  return results
    .map(r => `  "${r.string}" → ${r.accepted ? 'ACCEPT' : 'REJECT'}`)
    .join('\n');
}

const PROBE_PROMPT = (ctx: string, seen: ProbeResult[]) => {
...
const PREDICT_PROMPT = (ctx: string, results: ProbeResult[]) =>
...
const SUMMARISE_PROMPT = (prediction: string, alphabet: string[]) =>
...
export default function GeminiPanel() {
  ...
  const handleSummarize = async () => {
    ...
    try {
      ...
      console.log('[GeminiPanel] Results1:', probeRes1);
      ...
      console.log('[GeminiPanel] Results2:', probeRes2);
